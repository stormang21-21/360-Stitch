...
class ExportService: ObservableObject {
    ...
    func exportToUSDZ(meshAnchors: [ARMeshAnchor]) async -> URL? {
        ...
        print("📦 Asset contains \(meshAnchors.count) meshes")
        ...
        if FileManager.default.fileExists(atPath: url.path) {
            ...
            print("✅ USDZ saved: \(url.path) (\(fileSize ?? 0) bytes)")
            exportProgress = 1.0
            exportMessage = "✅ Exported! (\(meshAnchors.count) surfaces)"
            exportedURL = url
        } else {
            exportMessage = "❌ File not created"
            print("❌ Export failed: file not created at \(url.path)")
        }
